...
std::vector<int> sort_vector(std::vector<int> vector) {
	...
	std::string answer;
	vector.pop_back();
	std::cout << "Введите 'lower', если хотите отсортировать вектор по убыванию и 'greater' - если по возрастанию. Ввод: ";
	while (true) {
		std::cin >> answer;
		if (answer == "lower") {
			...
			std::cout << "Неверно введена команда. Повторите попытку. Ввод: ";
		}
	}
...
std::vector<int> insert_number(std::vector<int> vector) {
	...
	int index;
	int number;
	std::cout << "Введите позицию, на которую вы хотите вставить элемент: ";
	while (true) {
		std::cin >> index;
		std::cin.ignore();
		if (index > 0 && index < vector.size()) {
			...
			std::cout << "Вы вышли за пределы вектора. Повторите попытку: ";
		}
	}
	std::cout << "Введите число, которое вы хотите вставить: ";
	std::cin >> number;
	index--;
...
int find(std::vector<int> vector) {
	int index;
	std::vector<int>::iterator iter = vector.begin();
	std::cout << "Введите индекс элемента, который вы хотите получить: ";
	while (true) {
		std::cin >> index;
		if (index > 0 && index < vector.size()) {
			break;
